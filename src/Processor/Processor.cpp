...
void Processor::nmi()
{
    if (mCPU->memoryMap.mPPURegs->ppuctrl.nmi_enable == 1)
    {
        ...
        mCPU->memoryMap.mMirror->stack[--mRegisters.sp] = buf.h;
        mCPU->memoryMap.mMirror->stack[--mRegisters.sp] = buf.l;
        mCPU->memoryMap.mMirror->stack[--mRegisters.sp] = mRegisters.flags.raw;
    ...
    mInstructions =
        {
            ...
            /* 0 */ {make_shared<Brk>(mCPU, make_shared<Empty>(), &mRegisters.sp, &mRegisters.flags),
                     ...
                     nullptr,
                     make_shared<PushProcStatus>(mCPU, &mRegisters.flags, &mRegisters.sp),
