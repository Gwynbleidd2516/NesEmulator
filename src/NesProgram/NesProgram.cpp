...
void NesProgram::loadFile(string path)
{
    ...
    file.read(reinterpret_cast<char *>(&mCPU.rom), sizeof(CPU::rom) * mNesFile.head.rpgSize / 2);
    file.read(reinterpret_cast<char *>(&mCPU.ram), sizeof(CPU::ram) * mNesFile.head.chrSize);
